...
function animateHeroPipeline() {
  ...
  if (pipeline) {
    pipeline.addEventListener('mouseenter', () => { paused = true; });
    pipeline.addEventListener('mouseleave', () => { paused = false; });
  }
...
function initPipelineDiagram() {
  const steps = Array.from(document.querySelectorAll('.pipeline-diagram .pipeline-step'));
  ...
  steps.forEach((step, i) => {
    ...
    step.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
        ...
        if (prev) { prev.focus(); activate(prev); }
      }
    });
  });

  if (steps[0]) setTimeout(() => activate(steps[0]), 1200);
}
